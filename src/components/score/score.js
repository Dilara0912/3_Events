...
export class Score {
  ...
  createScoreDisplay() {
    ...
    this.missesDisplay.innerHTML = `Осталось пропусков гоблина: ${this.misses}`;
  ...
  addMiss() {
    ...
    this.missesDisplay.innerHTML = `Осталось пропусков гоблина: ${remain}`;
  ...
  handleCellClick(clickedIndex) {
    ...
    if (parseIntClickedIndex === this.goblin.currentPosition) {
      //значит попали и нужно засчитать +1
      ...
      this.goblin.resetGameInterval(); //перезапускаем интервал, чтобы отсчет 2 секунд начался заново
    } else {
      this.addMiss();
    }
  }
  ...
  setupModal() {
    ...
    if (!this.modalHandlerCreated) {
      //Создаем обработчик ТОЛЬКО если его еще нет
